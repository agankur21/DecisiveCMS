...
function MAIN()
{
   ...
   ${SPARK_HOME}/bin/spark-shell --master ${MASTER}  --jars ${JOB_JAR}  --executor-memory 3g --driver-memory 3g --conf spark.cassandra.connection.host=10.2.3.10 --conf spark.cassandra.input.split.size_in_mb=1500 --conf spark.shuffle.spill=false --conf "spark.executor.extraJavaOptions=-XX:+PrintGCDetails -XX:+PrintGCTimeStamps -XX:+UseCompressedOops";
